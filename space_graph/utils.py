...
def inv_sig_diag_new(
    Y: np.ndarray, beta: np.ndarray, center: bool = False
) -> np.ndarray:
    '''
    `InvSig.diag.new`: 1 / colMeans((Y - Y @ Beta0)^2) with diag(Beta)=0.

    ``center=True`` column-centers ``Y`` first; use when the caller may pass
    uncentered data (e.g. ``SPACE.fit`` with ``standardize=False``). Default
    ``False`` preserves the reference R semantics.
    '''
    b = beta.copy()
